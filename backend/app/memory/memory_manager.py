...
def write_long_term(openai_api_key: str, contact_id: str, text: str, kind: str = "note") -> str:
    vs = get_vectorstore(openai_api_key)
    doc_id = f"{contact_id}:{kind}"
    vs.add_texts([text], metadatas=[{"contact_id": contact_id, "kind": kind}], ids=[doc_id])
    vs.persist()
    return doc_id

def search_long_term(openai_api_key: str, contact_id: str, query: str, k: int = 4) -> list[dict]:
    vs = get_vectorstore(openai_api_key)
